...
from dnfdataload import train_dataset_prepare
...
if __name__ == "__main__":
    ...
    parser.add_argument('--batch-size', type=int, default=64, help='input batch size for training (default: 300)')
    parser.add_argument('--epochs', type=int, default=35, help='number of epochs to train (default: 1000)')
    parser.add_argument('--lr', type=float, default=0.003, help='learning rate (default: 0.003)')
    parser.add_argument('--num-blocks', type=int, default=6, help='number of invertible blocks (default: 10)')
    ...
    train_path = '/home/daip/share/old_share/wxf/Tea_cake_CBIR/npz/bird_train_dense.npz'
    test_path = '/home/daip/share/old_share/wxf/Tea_cake_CBIR/npz/bird_test_dense.npz'
    # load data
    print("loading data ...")
    t0_dataset, t2_dataset = train_dataset_prepare(train_path=train_path,test_path=test_path)
    ...
    if not os.path.exists('./z_trn_2022_7_20'):
        os.mkdir('./z_trn_2022_7_20');
    if not os.path.exists('./z_Sitwn_2022_7_20'):
        os.mkdir('./z_Sitwn_2022_7_20');
    if not os.path.exists('./chkptn2022_7_20'):
        os.mkdir('./chkptn2022_7_20');

    # initial
    print('\nDrawing tsne of bird ......')
    dnftsne.main('/home/daip/share/old_share/wxf/Tea_cake_CBIR/npz/bird_train_dense.npz', -1)
    ...
    for epoch in range(args.epochs):
        ...
        torch.save(model, './chkptn2022_7_20/model_epoch{}.pt'.format(epoch))

        # evaluation
        with torch.no_grad():
                print('do evaluation ......\n')
                path0 = './z_trn_2022_7_20/z0_epoch{}.npz'.format(epoch)
                u0, _ = model(t0_dataset[:][0].to(device))
                # u0, _ = model(t0_dataset[:][0])
                print("epoch",model)
                label0 = np.load('/home/daip/share/old_share/wxf/Tea_cake_CBIR/npz/bird_train_dense.npz')['utt']
                ...
                path2 = './z_Sitwn_2022_7_20/z2_epoch{}.npz'.format(epoch)
                u2, _ = model(t2_dataset[:][0].to(device))
                # u2, _ = model(t2_dataset[:][0])
                label2 = np.load('/home/daip/share/old_share/wxf/Tea_cake_CBIR/npz/bird_test_dense.npz')['utt']
